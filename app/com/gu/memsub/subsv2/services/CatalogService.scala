...
import scalaz.{EitherT, NonEmptyList, Validation, ValidationNel, \/}
...
object FetchCatalog {
  def fromS3[M[_]: scalaz.Monad](zuoraEnvironment: String, s3Client: S3Client = AwsS3.client): M[String \/ JsValue] = {
    import scalaz._
    import Scalaz._

    val request = GetObjectRequest.builder()
      .bucket("gu-zuora-catalog")
      .key(s"PROD/Zuora-" + zuoraEnvironment + "/catalog.json")
      .build()

    val result: String \/ JsValue = Try {
      ...
      case scala.util.Success(js) => \/-(js)
      case scala.util.Failure(ex) => -\/(ex.getMessage)
    }

    result.point[M] // uses Scalaz Monad
  }
}
...
class CatalogService[M[_] : Monad](productIds: ProductIds, fetchCatalog: M[String \/ JsValue], unsafeGet: M[Catalog] => Catalog, stage: String) {
  ...
  lazy val catalog: M[NonEmptyList[String] \/ Catalog] = {
    val plansM: M[String \/ List[CatalogZuoraPlan]] = joinUp(fetchCatalog)

    (for {
      plans <- EitherT(plansM).leftMap(_.wrapNel)
      catalog <- EitherT(validatePlans(plans).toDisjunction.point[M])
    } yield catalog).run
  }

  lazy val unsafeCatalog: Catalog = unsafeGet(
    catalog.map(_.valueOr(e => throw new IllegalStateException(s"$e while getting catalog")))
  )
    ...
    def joinUp[M[_]: scalaz.Monad](fetchCatalog: M[String \/ JsValue]): M[String \/ List[CatalogZuoraPlan]] = {
      import scalaz._
      import Scalaz._

      (for {
        catalog <- EitherT(fetchCatalog)
        catalogPlans <- EitherT(Json.fromJson[List[CatalogZuoraPlan]](catalog)
          .asEither
          .toDisjunction
          .leftMap(_.toString)
          .point[M])
